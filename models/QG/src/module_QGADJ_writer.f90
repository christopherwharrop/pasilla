...
!  use NetCDF_Writer, only : netcdf_writer_type
  ...
  public :: qgadj_writer_type

  type :: qgadj_writer_type
      private
...
!      procedure :: generic_write
  ...
  end type qgadj_writer_type

  interface qgadj_writer_type
...
contains
  ...
  ! constructor
  !
  ! Returns an initialized qgadj_writer_type object
  !------------------------------------------------------------------
  type(qgadj_writer_type) function constructor(io_format)
...
!        constructor%io_writer = ascii_writer_type()
  ...
  ! destructor
  !
  ! Deallocates pointers used by a qgadj_writer_type object (none currently)
  !------------------------------------------------------------------
  elemental subroutine destructor(this)

    type(qgadj_writer_type), intent(inout) :: this

    ! No pointers in qgadj_writer_type object so we do nothing
  ...
  subroutine write(this, model, filename)
    ...
    class(qgadj_writer_type), intent(inout) :: this
    class(qg_adj_type),  intent(   in) :: model
!    class(abstract_model_type),  intent(   in) :: model
    character(len=*),      intent(   in) :: filename

!    select type(model)

!      class is(qg_adj_type)
...
!  end subroutine generic_write
  ...
  subroutine ascii_write(this, model, filename)
    ...
    class(qgadj_writer_type), intent(in) :: this
    class(qg_adj_type),  intent(in) :: model
    character(len=*),      intent(in) :: filename
