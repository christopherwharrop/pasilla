...
40  FORMAT(A8,3I5,2F10.4)
  ...
  subroutine var_solver(bkg_cov,hrh_cov,bht_ino,jvc_for,background,anl_vec)
    ...
    class(Background_Type)      :: background
...
!$OMP END PARALLEL
  ...
  subroutine put_anl_vec(anl_vec,background)
...
40  FORMAT(A8,2I5,3F10.4)
    do t=1,tim_len
       do i=1,bkg_len
          write(*,40) "FIN",t,i,anl_vec(t,i),background%state(t,i),50.0+50.0*sin(((20.0*float(t-1)+float(i))/1000.0)*PI)
!         FOR 3DVAR
          if(mthd.le.2) write(*,40) "FIN",2,i,anl_vec(t,i),background%state(t,i),50.0+50.0*sin(((20.0*float(2-1)+float(i))/1000.0)*PI)
       end do
    end do
